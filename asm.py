...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='CPU-CPU3650 assembler')
    parser.add_argument('-A', metavar='addr', type=lambda s: int(s,0), default=0,
                        help='Starting address')
    parser.add_argument('-l', metavar='line', type=str, default=None,
                        help='single Line to assemble')
    parser.add_argument('infile', nargs='?', type=str,
                            default=None, help='Input file to assemble')
    ...
    if args.l != None:
        ...
        do_line(l)
        print(args.l, ": ", end='')
        for a in range(args.A, current_addr, 2):
            w = memory[a] | (memory[a+1] << 8)
            print("%04x" % w, end=' ')
        print()
        exit(0)

    if not (args.infile and args.infile.endswith('.asm') and os.access(args.infile, os.R_OK)):
        print('Missing input .asm file')
        args.print_usage()
        exit(1)
        
    outfile = args.infile.replace('.asm', '.bin')
